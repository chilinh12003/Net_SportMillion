...
{
    public class Sub_TrafficHN
    {
        MyExecuteData mExec;
        MyGetData mGet;

        public Sub_TrafficHN()
        {
            mExec = new MyExecuteData();
            mGet = new MyGetData();
        }

        public Sub_TrafficHN(string KeyConnect_InConfig)
        {
        ...
        public DataSet CreateDataSet()
        {
            try
            {
                string[] mPara = { "Type" };
                string[] mValue = { "0" };
                DataSet mSet = mGet.GetDataSet("Sp_Sub_TrafficHN_Select", mPara, mValue);
        ...
        public DataTable Select(int Type, string Para_1)
        {
            try
            {
                ...
                return mGet.GetDataTable("Sp_Sub_TrafficHN_Select", mPara, mValue);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public bool Insert(int? Type, string XMLContent)
        {
            try
            {
                ...
                if (mExec.ExecProcedure("Sp_Sub_TrafficHN_Insert", mpara, mValue) > 0)
                {
                    return true;
                }
                else
        ...
        public bool Delete(int? Type, string XMLContent)
        {
            try
            {
                string[] mpara = { "Type", "XMLContent" };
                string[] mValue = { Type.ToString(), XMLContent };
                if (mExec.ExecProcedure("Sp_Sub_TrafficHN_Delete", mpara, mValue) > 0)
                {
                    return true;
        ...
        public bool Update(int? Type, string XMLContent)
        {
            try
            {
                string[] mpara = { "Type", "XMLContent" };
                string[] mValue = { Type.ToString(), XMLContent };
                if (mExec.ExecProcedure("Sp_Sub_TrafficHN_Update", mpara, mValue) > 0)
                {
                    return true;
                }
                else
