...
{
    public partial class Ad_Match : MyASPXBase
    {
        ...
        int Admin_Paging1_GetTotalPage_Callback_Change()
        {
            try
            {
                ...
                return mMatch.TotalRow(SearchType, str_SearchContent, StatusID);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        DataTable Admin_Paging1_GetData_Callback_Change()
        {
            try
            {
                ...
                return mMatch.Search(SearchType, Admin_Paging1.mPaging.BeginRow, Admin_Paging1.mPaging.EndRow, str_SearchContent, StatusID, SortBy);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        protected void lbtn_Delete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mMatch.Delete(0, dds_Parent.GetXml()))
                {
                    #region Log member
                    MemberLog mLog = new MemberLog();
                    MemberLog.ActionType Action = MemberLog.ActionType.Delete;
                    mLog.Insert("Match", string.Empty, dds_Parent.GetXml(), Action, true, string.Empty);
                    #endregion
